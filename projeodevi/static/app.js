// API endpoint'leri
const API_URL = ''; // API endpointleri doğrudan '/' ile başlıyor

// DOM elementleri
const authContainer = document.getElementById('auth-container'); // Bu element index.html'de var
const mainContainer = document.getElementById('main-container'); // Bu element yok, kaldırılabilir veya ilgili sayfalara eklenebilir
const cartContainer = document.getElementById('cart-container'); // Bu element yok, kaldırılabilir veya ilgili sayfalara eklenebilir
...
const logoutBtn = document.getElementById('logout-btn'); // logout-btn olarak düzeltildi
...
async function loginAdmin(email, password) {
    try {
        ...
        if (response.ok) {
            ...
            throw new Error(data.msg || 'Admin girişi başarısız');
        }
    } catch (error) {
        console.error('Admin giriş hatası:', error);
        alert(error.message || 'Admin girişi yapılırken bir hata oluştu');
    }
}
    ...
    if (pathname === '/') {
        ...
        if (registerForm) {
            registerForm.onsubmit = async (e) => {
                ...
                await registerUser(email, password, interests);
            };
        }
        ...
        if (showLoginBtn) showLoginBtn.onclick = () => showForm('loginForm');
        if (showRegisterBtn) showRegisterBtn.onclick = () => showForm('registerForm');
        if (showAdminBtn) showAdminBtn.onclick = () => showForm('adminForm');
    }

    // Form görünürlüğünü sıfırla
    function resetForms() {
    ...
    async function loadEvents() {
        ...
        try {
            ...
            if (!response.ok) {
                 ...
                 alert(errorData.msg || 'Etkinlikler yüklenemedi');
                 if (response.status === 401) {
        ...
        try {
            const response = await fetchWithAuth('/api/announcements');
             ...
             if (!response.ok) {
                 ...
                 alert(errorData.msg || 'Duyurular yüklenemedi');
                  if (response.status === 401) {
                     logout(); // Yetkisiz erişimde çıkış yap
                 }
                 return;
        ...
        try {
            ...
            const response = await fetchWithAuth(`/api/weather-check/${eventId}`); // Korumalı API çağrısı
             ...
             if (!response.ok) {
                 ...
                 alert(errorData.msg || 'Hava durumu kontrolü başarısız');
                  if (response.status === 401) {
                     logout();
                 }
                 return false;
            ...
            console.error('Hava durumu kontrolü yapılırken hata oluştu:', error);
             alert('Hava durumu kontrolü yapılırken bir hata oluştu');
            return false;
        }
    }
        ...
        try {
            ...
            const response = await fetchWithAuth(`/api/events/${eventId}`);
             if (response.ok) {
                ...
                alert(errorData.msg || 'Etkinlik sepete eklenemedi.');
                 if (response.status === 401) {
                    logout();
                }
            }
            ...
            console.error('Sepete eklerken hata oluştu:', error);
             alert('Etkinlik sepete eklenirken bir hata oluştu.');
        }
    };
