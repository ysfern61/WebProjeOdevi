...
$query = "SELECT id, email, is_approved FROM users WHERE id = :user_id";
$stmt = $db->prepare($query);
...
// Approve user
$query = "UPDATE users SET is_approved = 1 WHERE id = :user_id";
$stmt = $db->prepare($query);
$stmt->bindParam(":user_id", $user_id);
...
if ($stmt->execute()) {
    ...
    echo json_encode(array(
        "success" => false,
        "message" => "Kullanıcı onaylanırken bir hata oluştu."
    ));
    return;
}
?>